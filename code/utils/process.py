...
def load_part(dataset_str, str, name, dice):
    '''
    base_path = "data_part\\"
    edges_path = base_path + dataset_str + "\\edges.txt" #done
    label_path = base_path + dataset_str + "\\labels.txt" 
    features_path = base_path + dataset_str + "\\features.txt" '''

    base_path = "./data_part/"
    edges_path = base_path + dataset_str +"/"+ str +  "/edges/" +name #done
    label_path = base_path + dataset_str +"/"+ str + "/labels/" +name
    features_path = base_path + dataset_str +"/"+ str + "/features/"+name
    weight_path = base_path + dataset_str +"/"+ str + "/weight_matrix/"+name
    ...
    weight_matrix = np.loadtxt(weight_path)
    weight_matrix = np.asarray(weight_matrix)
    weight_matrix = np.mat(weight_matrix)
    weight_matrix = weight_matrix.T
    weight_matrix = np.array(weight_matrix)
    
    import math
    shape = math.sqrt(weight_matrix.size)
    weight_matrix = weight_matrix.reshape(int(shape),int(shape))
    weight_matrix = np.round(weight_matrix, decimals=2)

    return adj, features, labels, kmeans_features_labels, idx_train, idx_val, idx_test, G, weight_matrix
...
### Amin Abidi
def dataframe(dataset_str,dice):
    
    """
    columns = ["adj", "features", "labels", "kmeans_features_labels", "idx_train", "idx_val", "idx_test", "G","weight_matrix"]
    
    df_ATP = pd.DataFrame(data = None  , columns = columns)
    df_NADH = pd.DataFrame(data= None , columns = columns)

    ATP = os.listdir(f'./data_part/{dataset_str}/ATP/edges')
    NADH = os.listdir(f'./data_part/{dataset_str}/NADH/edges')
    
    for entry in NADH:
        adj, features, labels, kmeans_features_labels, idx_train, idx_val, idx_test, G,weight_matrix = load_part(dataset_str, 'NADH', entry, dice)
        df_NADH.loc[len(df_NADH.index)] = [adj, features, labels, kmeans_features_labels, idx_train, idx_val, idx_test, G,weight_matrix]
        print(df_NADH)
        print(type(adj))
        print(type(features))
        print(type(labels))
    ...
    for entry in ATP:
        adj, features, labels, kmeans_features_labels, idx_train, idx_val, idx_test, G,weight_matrix = load_part(dataset_str, 'ATP', entry, dice)
        df_ATP.loc[len(df_ATP.index)] = [adj, features, labels, kmeans_features_labels, idx_train, idx_val, idx_test, G,weight_matrix]
        print(df_ATP)
    
    df_ATP["type"] = ["ATP" for _ in range(len(df_ATP))]
    df_NADH["type"] = ["NADH" for _ in range(len(df_NADH))]
    
    data = pd.concat([df_ATP , df_NADH])

    #dtypes = {'adj':type(adj), 'features':type(features), 'labels':type(labels), 'kmeans_features_labels':type(kmeans_features_labels), 'idx_train':type(idx_train), 'idx_val':type(idx_val), 'idx_test':type(idx_test), 'G':type(G), 'weight_matrix':type(weight_matrix)}
...
def get_labelmatrix(labels): #ground truth labels 
    node_num = labels.shape[0]
